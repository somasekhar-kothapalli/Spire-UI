...
export default class SPopover extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SPopover.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SPopover.#shadowTemplate.content, true)
    );
    ...
    this["#backdrop"].addEventListener("click", (event) => {
      // Don't close a <dialog> when user clicks <s-popover> backdrop inside it.
    ...
    this["#backdrop"].addEventListener("pointerdown", (event) => {
      ...
      if (this.hasAttribute("animating")) {
        ...
        this.parentNode &&
        this.parentNode.localName !== "s-button" &&
    ...
    return new Promise(async (resolve) => {
      ...
      if (this.opened === false) {
        ...
        window.addEventListener(
          "resize",
          (this.#windowResizeListener = () => this.#onResize())
        );
        Spire.addEventListener(
          "themechange",
          (this.#themeChangeListener = () => this.#updateArrowStyle())
        );
  ...
  close(animate = true) {
    return new Promise(async (resolve) => {
      if (this.opened === true) {
        ...
        Spire.removeEventListener("themechange", this.#themeChangeListener);
        this["#backdrop"].hide();
        this.dispatchEvent(
    ...
    // Position the popover
    {
      if (align === "bottom" || align === "top") {
        let positionBottom = (reduceHeight = false) => {
          ...
          bottomOverflow =
            popoverRect.bottom + windowWhitespace - window.innerHeight;
        ...
        let floatCenter = () => {
          ...
          this["#arrow"].style.left =
            extraLeft + contextRect.left + contextRect.width / 2 + "px";
          this.style.left =
            extraLeft +
            contextRect.left +
            contextRect.width / 2 -
          ...
          rightOverflow =
            popoverRect.right + windowWhitespace - window.innerWidth;
        ...
        let floatCenter = () => {
          ...
          this["#arrow"].style.top =
            extraTop + contextRect.top + contextRect.height / 2 + "px";
          this.style.top =
            extraTop +
            contextRect.top +
...
customElements.define("s-popover", SPopover);
