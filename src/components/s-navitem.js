...
export default class SNavItem extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SNavItem.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SNavItem.#shadowTemplate.content, true)
    );
  ...
  connectedCallback() {
    Spire.whenThemeReady.then(() => {
      this._updateArrowPathData();
    });

    Spire.addEventListener(
      "themechange",
      (this.#xelThemeChangeListener = () => this._updateArrowPathData())
    );
  ...
  _onClick(event) {
    if (
      this === event.target.closest("s-navitem") &&
      ...
      if (event.target.localName !== "s-nav") {
        if (this.expanded) {
          ...
          let event = new CustomEvent("expand", {
            bubbles: true,
            cancelable: true,
          });
          this.dispatchEvent(event);
...
customElements.define("s-navitem", SNavItem);
