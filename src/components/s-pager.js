...
// @part item
// @part toggled-item
export class SPager extends HTMLElement {
  ...
  static #shadowStyleSheet = css`
    :host {
      ...
      --first-path-data: M 16 20 L 26 20 L 26 80 L 16 80 L 16 20 Z M 84 20 L 84
        80 L 36 50 L 84 20 Z;
      --last-path-data: M 84 20 L 74 20 L 74 80 L 84 80 L 84 20 Z M 16 20 L 16
        80 L 64 50 L 16 20 Z;
    }
    :host([hidden]) {
      display: none;
    }
  ...
  connectedCallback() {
    if (!this.#shadowRoot) {
      ...
      this.#shadowRoot.adoptedStyleSheets = [SPager.#shadowStyleSheet];
      this.#shadowRoot.append(
        document.importNode(SPager.#shadowTemplate.content, true)
      );
    ...
    Spire.whenThemeReady.then(() => this.#update());
  }
  ...
  #getItemsHTML() {
    ...
    if (this.controls.includes("next") && this.max > 1) {
      itemsHTML += this.#getNamedItemHTML(
  ...
  #update() {
    ...
    // Update items
    {
      ...
      if (oldFocusedItem) {
        let item = this["#items"].querySelector(
          `#${oldFocusedItem.id}:not([data-disabled])`
        );
    ...
    // Update arrow paths
    {
      ...
      for (let pos of ["prev", "next", "first", "last"]) {
        ...
        if (path) {
          path.setAttribute(
            "d",
            computedStyle.getPropertyValue(`--${pos}-path-data`)
          );
        }
      }
    }
...
customElements.define("s-pager", SPager);
