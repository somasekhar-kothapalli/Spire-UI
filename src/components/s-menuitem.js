...
export default class SMenuItem extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SMenuItem.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SMenuItem.#shadowTemplate.content, true)
    );
  ...
  connectedCallback() {
    ...
    Spire.addEventListener(
      "themechange",
      (this.#xelThemeChangeListener = () => this.#onThemeChange())
    );
  }
  ...
  #updateCheckmarkPathData() {
    let pathData = getComputedStyle(this["#checkmark"]).getPropertyValue(
      "--path-data"
    );
  ...
  #updateArrowPathData() {
    let pathData = getComputedStyle(this["#arrow"]).getPropertyValue(
      "--path-data"
    );
  ...
  #updateArrowIconVisibility() {
    if (this.parentElement && this.parentElement.localName === "s-menubar") {
  ...
  async #onPointerDown(pointerDownEvent) {
    ...
    if (this.matches("[closing] s-menuitem")) {
      pointerDownEvent.preventDefault();
      pointerDownEvent.stopPropagation();
      return;
    }

    if (pointerDownEvent.target.closest("s-menuitem") !== this) {
      return;
    }
    ...
    // to last at least 150ms.
    {
      ...
      this.addEventListener(
        ...
        (pointerUpOrCancelListener = async () => {
          ...
          let minPressedTime =
            pointerDownEvent.pointerType === "touch" ? 600 : 150;
  ...
  async #onClick(event) {
    ...
    let clickedMenuItem = event.target.closest("s-menuitem");
    let clickedMenu = event.target.closest("s-menu");

    if (
      clickedMenuItem !== this ||
      clickedMenu !== this.closest("s-menu") ||
      this.matches("[closing] s-menuitem")
    ) {
      return;
    }
    ...
    if (
      event.detail > 0 &&
      rectContainsPoint(
        clickedMenuItem.getBoundingClientRect(),
    ...
    // Trigger effect
    if (!this.querySelector(":scope > s-menu")) {
