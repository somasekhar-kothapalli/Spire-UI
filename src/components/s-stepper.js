...
export default class SStepper extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SStepper.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SStepper.#shadowTemplate.content, true)
    );
  ...
  connectedCallback() {
    ...
    Spire.addEventListener(
      "themechange",
      (this.#xelThemeChangeListener = () => this.#updatePathData())
    );
  ...
  disconnectedCallback() {
    ...
    if (this.#parentInput) {
      if (this.#parentInput.querySelector(":scope > s-stepper") === null) {
  ...
  #updatePathData() {
    let incrementPathData = getComputedStyle(
      this["#increment-arrow"]
    ).getPropertyValue("--path-data");
    let decrementPathData = getComputedStyle(
      this["#decrement-arrow"]
    ).getPropertyValue("--path-data");
...
customElements.define("s-stepper", SStepper);
