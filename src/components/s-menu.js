...
export default class SMenu extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SMenu.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SMenu.#shadowTemplate.content, true)
    );
  ...
  openOverElement(underElement, overElement) {
    return new Promise(async (resolve) => {
      let items = this.querySelectorAll(":scope > s-menuitem");
    ...
    return new Promise(async (resolve) => {
      ...
      if (items.length > 0) {
        ...
        let item = [...items].find((item) => {
          let itemLabel = item.querySelector("s-label");
        ...
        let overLabel = item.querySelector("s-label");
    ...
    return new Promise(async (resolve) => {
      ...
      if (direction === "horizontal") {
        ...
        // Move the menu horizontally if it overflows the right screen edge
        {
          ...
          if (
            menuBounds.left + menuBounds.width + WINDOW_PADDING >
            window.innerWidth
          ) {
        ...
        // Move the menu vertically it overflows the bottom screen edge
        {
          ...
          if (
            menuBounds.top + menuBounds.height + WINDOW_PADDING >
            window.innerHeight
          ) {
            let bottomOverflow =
              menuBounds.top +
        ...
        if (
          ...
          element.parentElement.localName === "s-menubar"
        ) {
          let menuBounds = this.getBoundingClientRect();
        ...
        if (align === "start") {
          ...
          // Float the menu to the right screen edge if it overflows the right screen edge
          {
            ...
            if (
              ...
              this.style.left =
                window.innerWidth - WINDOW_PADDING + extraLeft + "px";
            }
          }
        }
  ...
  openAtPoint(left, top) {
    return new Promise(async (resolve) => {
      ...
      let messageElements = [
        ...this.querySelectorAll(":scope > s-menuitem > s-label > s-message"),
      ];
      ...
      if (menuBounds.bottom + WINDOW_PADDING > window.innerHeight) {
        ...
        if (menuBounds.top < WINDOW_PADDING) {
          ...
          this.style.top = top + extraTop + "px";
          this.style.height =
            window.innerHeight - WINDOW_PADDING - WINDOW_PADDING + "px";
        }
      }
  ...
  close(animate = true) {
    return new Promise(async (resolve) => {
      if (this.opened) {
        ...
        let item = this.closest("s-menuitem");
        ...
        for (let item of this.querySelectorAll(":scope > s-menuitem")) {
          let submenu = item.querySelector("s-menu[opened]");
  ...
  focusNextMenuItem() {
    ...
    if (refItem) {
      ...
      if (nextItem) {
        ...
        let menu = refItem.querySelector("s-menu");
  ...
  focusFirstMenuItem() {
    let items = this.querySelectorAll(
      "s-menuitem:not([disabled]):not([hidden])"
    );
  ...
  #delay(callback) {
...

    {
      ...
      let openedSubmenu = this.querySelector("s-menu[opened]");
  ...
  #isClosing() {
    return this.matches("*[closing], *[closing] s-menu");
  }
  ...
  #onPointerDown(event) {
    ...
    if (
      ...
      event.target.closest("s-menu") === this
    ) {
      ...
      // Focus and expand the menu item under pointer and collapse other items
      {
        ...
        if (
          item &&
          item.disabled === false &&
          item.closest("s-menu") === this
        ) {
          if (item.matches(":focus") === false) {
            this.#delay(async () => {
              let otherItem = this.querySelector(":scope > s-menuitem:focus");

              if (otherItem) {
                let otherSubmenu = otherItem.querySelector("s-menu");
              ...
              let menu = item.closest("s-menu");
              let submenu = item.querySelector("s-menu");
              let otherItems = [
                ...this.querySelectorAll(":scope > s-menuitem"),
              ].filter(($0) => $0 !== item);
  ...
  #onPointerOver(event) {
    ...
    if (event.target.closest("s-menu") === this) {
      if (this.#isPointerOverMenuBlock === false) {
        this.#onMenuBlockPointerEnter();
      }
  ...
  #onPointerOut(event) {
    ...
    if (
      !event.relatedTarget ||
      closest(event.relatedTarget, "s-menu") !== this
    ) {
  ...
  #onKeyDown(event) {
    if (this.#isClosing()) {
      ...
      let focusedItem = this.querySelector("s-menuitem:focus");

      if (focusedItem) {
        let submenu = focusedItem.querySelector("s-menu");
      ...
      if (focusedItem) {
        let parentMenu = focusedItem.closest("s-menu");
        let parentItem = parentMenu.closest("s-menuitem");

        if (parentItem && parentItem.closest("s-menu")) {
          event.preventDefault();
          event.stopPropagation();
...
customElements.define("s-menu", SMenu);
