...
// @element s-message
export default class SMessage extends HTMLElement {
  ...
  get args() {
    ...
    let serializedArgs = this.hasAttribute("args")
      ? this.getAttribute("args").trim()
      : "";
  ...
  set args(args) {
    let serializedArgs = Object.keys(args)
  ...
  set ellipsis(ellipsis) {
    ellipsis
      ? this.setAttribute("ellipsis", "")
      : this.removeAttribute("ellipsis");
  }
  ...
  disconnectedCallback() {
    Spire.removeEventListener("localeschange", this.#localesChangeListener);
  ...
  async #update() {
    await Spire.whenLocalesReady;

    let message = Spire.queryMessage(this.href, this.args);
    ...
    if (message.fallback && this.#defaultContent) {
      ...
      if (message.format === "html") {
        ...
        if (
          ...
          Spire.autocapitalize === "titlecase"
        ) {
          this.textContent =
            toTitleCase(message.content) + (this.ellipsis ? "…" : "");
        } else {
...
customElements.define("s-message", SMessage);
