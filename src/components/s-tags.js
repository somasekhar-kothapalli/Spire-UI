...
// @element s-tags
// @event toggle
export default class STags extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [STags.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(STags.#shadowTemplate.content, true)
    );
  ...
  #onKeyDown(event) {
    if (event.defaultPrevented === false) {
      if (
        ...
        let focusedTag = this.querySelector("s-tag:focus");
        ...
        if (focusedTag) {
          if (focusedTag.nextElementSibling) {
            ...
            focusedTag !== focusedTag.parentElement.firstElementChild
          ) {
...
customElements.define("s-tags", STags);
