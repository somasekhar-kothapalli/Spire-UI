...
// @event toggle
// @event beforetoggle
export default class SDrawer extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SDrawer.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SDrawer.#shadowTemplate.content, true)
    );
    ...
    return new Promise(async (resolve) => {
      ...
      if (this.position === "left") {
        this.#openAnimation = this.animate(
          {
            transform: [
              `translateX(-${drawerRect.right}px)`,
              "translateX(0px)",
            ],
          },
        ...
        this.#openAnimation = this.animate(
          {
            transform: [
              `translateY(-${drawerRect.bottom}px)`,
              "translateY(0px)",
            ],
          },
          { duration: transitionDuration, easing: transitionTimingFunction }
        ...
        this.#openAnimation = this.animate(
          {
            transform: [
              `translateY(${drawerRect.height}px)`,
              "translateY(0px)",
            ],
          },
          { duration: transitionDuration, easing: transitionTimingFunction }
      ...
      window.addEventListener(
        ...
        (this.#windowClickListener = (event) => {
          if (this.#openAnimation.playState === "finished" && this.popover) {
            ...
            if (
              rectContainsPoint(popoverRect, {
                x: event.clientX,
                y: event.clientY,
              }) === false
            ) {
              let pointerEvents = getComputedStyle(
                this,
                "::backdrop"
              ).pointerEvents;
  ...
  close() {
    return new Promise(async (resolve) => {
      ...
      let transitionTimingFunction =
        computedStyle.getPropertyValue("transition-timing-function") || "ease";
      let drawerRect = this.getBoundingClientRect();
        ...
        this.#closeAnimation = this.animate(
          {
            transform: [
              "translateY(0px)",
              `translateY(-${drawerRect.bottom + 50}px)`,
            ],
          },
          { duration: transitionDuration, easing: transitionTimingFunction }
        );
...
customElements.define("s-drawer", SDrawer);
