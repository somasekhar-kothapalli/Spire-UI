...
export default class SSelect extends HTMLElement {
  ...
  static #shadowStyleSheet = css`
    ...
    #button > #arrow-container #arrow {
      ...
      color: currentColor;
      --text-color: 50%;
      --path-data: M 25 41 L 50 16 L 75 41 L 83 34 L 50 1 L 17 34 Z M 17 66 L 50
        100 L 83 66 L 75 59 L 50 84 L 25 59 Z;
    }
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SSelect.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SSelect.#shadowTemplate.content, true)
    );
  ...
  connectedCallback() {
    ...
    Spire.addEventListener(
      "themechange",
      (this.#xelThemeChangeListener = () => this.#updateArrowPathData())
    );
  }
  ...
  #expand() {
    ...
    // Reduce menu width if it oveflows the right client bound
    {
      ...
      if (menuBounds.right + WINDOW_PADDING > window.innerWidth) {
        this.style.maxWidth =
          window.innerWidth - menuBounds.left - WINDOW_PADDING + "px";
      }
    }
  }
  ...
  #updateButtonChildrenSize() {
    for (let buttonChild of this["#button"].children) {
      if (buttonChild !== this["#arrow-container"]) {
        let { width, height, margin, padding, border, borderRadius } =
          getComputedStyle(buttonChild[$itemChild]);

        if (
          ["s-icon", "x-swatch", "img", "svg"].includes(
...
customElements.define("x-select", SSelect);
