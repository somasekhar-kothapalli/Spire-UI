...
// @part input
// @part suggestions
export default class STagsInput extends HTMLElement {
  ...
  static #shadowStyleSheet = css`
    ...
    #suggestions-popover s-tag {
      display: block;
      margin: 4px 0 0 0;
    }
    #suggestions-popover s-tag:first-child {
      margin-top: 0;
    }
  `;
  ...
  // @readOnly
  get value() {
    let tags = [...this.children].filter(
      (child) => child.localName === "s-tag"
    );
  ...
  set suggestions(suggestions) {
    suggestions
      ? this.setAttribute("suggestions", "")
      : this.removeAttribute("suggestions");
  }
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [
      ...
      STagsInput.#shadowStyleSheet,
    ];
    this.#shadowRoot.append(
      document.importNode(STagsInput.#shadowTemplate.content, true)
    );
    ...
    if (tagText.length > 0) {
      if (this.value.includes(tagText) === false) {
        ...
        if (tagText.includes(":")) {
          ...
          tag = html`
            <s-tag class="tag" value="${tagText}" removable>
              <s-label slot="scope">${scopeText}</s-label>
              <s-label>${mainText}</s-label>
            </s-tag>
          `;
        }
        // Normal tag
  ...
  #onShadowRootPointerDown(event) {
    ...
    if (event.target === this["#input"]) {
      ...
      let tag = event.target.closest("s-tag");
    ...
    } else if (event.target.closest("s-tag")) {
      if (this["#input"].value !== "") {
        this.#commitInput();
        this.#updateSuggestions();
        this.#updatePlaceholderVisibility();
  ...
  #onInputInput(event) {
    ...
    if (value.includes(this.delimiter)) {
      ...
      this.#updateInputSize();
      this.#updateSuggestions(
        event.inputType === "deleteContentBackward" ? false : true
      );
    }
  }
  ...
  #onKeyDown(event) {
    if (event.code === "Enter" || event.code === "NumpadEnter") {
      ...
      } else if (event.target.localName === "s-tag") {
      ...
      if (this["#suggestions-popover"].opened) {
        ...
        if (suggestedTags.length > 1) {
          ...
          // Update input
          {
            let text = nextTag.querySelector("s-label").textContent;
        ...
        if (suggestedTags.length > 1) {
          ...
          // Determine the previous tag to select
          {
            ...
            if (currentTag) {
              ...
              let currentTagIndex = suggestedTags.indexOf(currentTag);
              prevTag =
                suggestedTags[currentTagIndex - 1] ||
          ...
          // Update input
          {
            let text = prevTag.querySelector("s-label").textContent;
            let startOffset = this["#input"].selectionStart;
  ...
  #updatePlaceholderVisibility() {
    let placeholder = this.querySelector(":scope > s-label");

    if (placeholder) {
      placeholder.hidden =
        this.value.length > 0 || this["#input"].value.length > 0;
    }
  }
  ...
  #updateInputSelection(
    startOffset = 0,
    endOffset = this["#input"].value.length
  ) {
    ...
    if (this.suggestions) {
      ...
      // Update input
      {
        if (suggestedTags.length > 0 && autocomplete === true) {
          ...
          let suggestedText =
            suggestedTags[0].querySelector("s-label").textContent;
...
customElements.define("s-tagsinput", STagsInput);
