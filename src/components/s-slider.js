...
export default class SSlider extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadowRoot.adoptedStyleSheets = [SSlider.#shadowStyleSheet];
    this.#shadowRoot.append(
      document.importNode(SSlider.#shadowTemplate.content, true)
    );
  ...
  #onThumbResize() {
    ...
    if (thumbRect.width !== this.#computedThumbWidth) {
      ...
      this["#main"].style.setProperty(
        "--computed-thumb-width",
        thumbRect.width + "px"
      );
    }
  }
  ...
  #onShadowRootPointerDown(pointerDownEvent) {
    ...
    let { width: thumbWidth, height: thumbHeight } =
      this["#start-thumb"].getBoundingClientRect();
    ...
    // Determine the thumb to be dragged
    {
      if (pointerDownEvent.target.matches(".thumb")) {
        ...
        if (this["#end-thumb"].hidden === true) {
          ...
          let pointerPoint = new DOMPoint(
            pointerDownEvent.clientX,
            pointerDownEvent.clientY
          );

          let startDistance = getDistanceBetweenPoints(
            pointerPoint,
            startPoint
          );
    ...
    let updateValue = (clientX, clientY) => {
      ...
      let value =
        (x / (containerBounds.width - thumbWidth)) * (this.max - this.min) +
        this.min;
    ...
    draggedThumb.addEventListener(
      ...
      (pointerUpOrCancelListener = () => {
        ...
        draggedThumb.removeEventListener(
          "pointerup",
          pointerUpOrCancelListener
        );
        draggedThumb.removeEventListener(
    ...
    // Normal
      ...
      // Range track
      {
        ...
        if (offset >= originOffset) {
          ...
          this["#range-track"].style.removeProperty(
            "border-bottom-left-radius"
          );
          this["#range-track"].style.setProperty(
      ...
      if (this.ticks) {
        ...
        for (
          ...
          this["#ticks"].insertAdjacentHTML(
            "beforeend",
            `<div class="tick" part="${parts.join(
              " "
            )}" data-value="${value}" style="left: ${left}%"></div>`
          );
        }
      }
    }
